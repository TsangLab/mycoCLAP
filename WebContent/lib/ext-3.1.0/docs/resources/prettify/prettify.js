...
(function() {
	...
	var K = aa, Sc = K + ba, T = Sc + ca, ta = T + da, ua = T + ea, Tc = ua
			+ fa, va = T + ga, wa = ha, xa = K + ia, ya = K + ja, za = K + ka, Uc = ta
			+ Tc + va + wa + xa + ya + za;
	function Vc(b) {
	...
	var Wc = (function() {
		var b = [pa, qa, ra, s, sa, Ha, v, Ia, Ja, Ka, La, Ma, Na, Oa, Pa, Qa,
				Ra, w, Sa, Ta, Ua, y, z, Va, Wa, Xa, Ya, Za, $a, A, ab, bb, cb,
				db, eb, fb, C, gb, hb, ib, jb, kb, lb, mb, nb, ob, pb, qb, rb,
				sb, tb, ub, vb, wb, xb, yb, zb, Ab, Bb, Cb], a = Db;
	...
	function ed(b) {
		...
		return b.replace(Zc, z).replace($c, A).replace(ad, Nb).replace(bd, G)
				.replace(cd, v).replace(dd, Ob)
	}
	function Da(b) {
		return Pb === b.tagName
	}
	...
	var md = E([], [
					...
					[P, /^<\?[\s\S]*?(?:\?>|$)/, null],
					[P, /^<%[\s\S]*?(?:%>|$)/, null],
					[P, /^<(script|style|xmp)\b[^>]*>[\s\S]*?<\/\1\b[^>]*>/i,
							null], [Q, /^<\/?\w[^<>]*>/, null]]);
	function nd(b) {
		var a = md(b);
		for (var c = 0; c < a.length; c += 2)
			if (a[c + 1] === P) {
				...
				var e = b.substring(d, g), h = e
						.match(/^(<[^>]*>)([\s\S]*)(<\/[^>]*>)$/);
				if (h)
					a.splice(c, 2, d, Q, d + h[1].length, P, d + h[1].length
									+ (h[2] || I).length, Q)
			}
		return a
	}
			...
			for (var j = 0; j < i.length; j += 2) {
				...
				if (m === J) {
					...
					for (var n = 0, q = l.length; n < q; n += 2) {
						var r = l[n + 1];
						if (r === J) {
							var B = l[n], M = n + 2 < q ? l[n + 2] : p.length, x = f
									.substring(B, M);
							if (x === ec)
								l[n + 1] = S;
							else if (x in d)
								l[n + 1] = fc;
							else if (/^@?[A-Z][A-Z$]*[a-z][A-Za-z$]*$/.test(x))
								l[n + 1] = x.charAt(0) === C ? Z : gc
						}
					}
					D(l, i, j, 2);
					j += l.length - 2
	...
	function ud(b) {
		var a = _pr_isIE6(), c = [document.getElementsByTagName(Lc),
				document.getElementsByTagName(Mc),
				document.getElementsByTagName(Nc)], d = [];
			...
			for (; h < d.length && (new Date).getTime() < i; h++) {
				...
				if (j.className && j.className.indexOf(Oc) >= 0) {
					...
					for (var k = j.parentNode; k; k = k.parentNode)
						if ((k.tagName === Lc || k.tagName === Mc || k.tagName === Nc)
								&& k.className && k.className.indexOf(Oc) >= 0) {
							o = true;
							break
						}
					if (!o) {
						...
						if (a && j.tagName === H) {
							...
							for (var x = M.length; --x >= 0;) {
								var Ga = M[x];
								Ga.parentNode.replaceChild(document
												.createTextNode(Rc), Ga)
							}
						}
					}
				}
