...
(function() {
	...
	isSafari3 = isSafari && check(/version\/3/), isSafari4 = isSafari
			...
			&& !isIE7 && !isIE8, isGecko = !isWebKit && check(/gecko/), isGecko2 = isGecko
	...
	Ext.apply(Ext, {
		 ...
		 * Ext.Element. Here are some examples:
		 * 
		 * <pre><code>
		...
		getDom : function(el) {
			...
			return el.dom ? el.dom : (Ext.isString(el)
...
Ext.apply(Ext, function() {
	...
	return {
		 ...
		 * with CSS background images. In older versions of IE, this defaults to
		 * "http://extjs.com/s.gif" and you should change this to a URL on your
		 * server. For other browsers it uses an inline data URL.
		 * 
		 * @type String
		 */
		...
		/**
		 * <p>
		 * Utility method for returning a default value if the passed value is
		 * empty.
		 * </p>
		 * <p>
		 * The value is deemed to be empty if it is<div class="mdetail-params">
		 ...
		 *            empty
		 * @param {Boolean}
		 *            allowBlank (optional) true to allow zero length strings to
		 *            qualify as non-empty (defaults to false)
		 ...
		 * ready. The event name is specified with an <tt>&#64;</tt> suffix.
		 * 
		 * <pre><code>
		 * Ext.addBehaviors({
		...
		addBehaviors : function(o) {
			if (!Ext.isReady) {
				...
				var cache = {}, // simple cache for applying multiple behaviors
								// to same selector does query multiple times
				parts, b, s;
				for (b in o) {
					if ((parts = b.split('@'))[1]) { // for Object prototype
														// breakers
						s = parts[0];
						if (!cache[s]) {
		...
		/**
		 * Utility method for getting the width of the browser scrollbar. This
		 * can differ depending on operating system settings, such as the theme
		 * or font size.
		 * 
		 * @param {Boolean}
		 *            force (optional) true to force a recalculation of the
		 *            value.
		...
		getScrollBarWidth : function(force) {
			...
			if (force === true || scrollWidth === null) {
				...
				var div = Ext
						.getBody()
						.createChild('<div class="x-hide-offsets" style="width:100px;height:50px;overflow:hidden;"><div style="height:200px;"></div></div>'), child = div
						.child('div', true);
				var w1 = child.offsetWidth;
				div.setStyle('overflow', (Ext.isWebKit || Ext.isGecko)
								? 'auto'
								: 'scroll');
		...
		/**
		 * Copies a set of named properties fom the source object to the
		 * destination object.
		 * <p>
		 * example:
		 * 
		 ...
		 * 				this.initialBox = Ext.copyTo({}, this.initialConfig,
		 * 						'x,y,width,height');
		 * 			}
		 * 		});
		 * </code></pre>
		 ...
		 *            The source object.
		 * @param {Array/String}
		 *            Either an Array of property names, or a comma-delimited
		 ...
		 * 
		 * @param {Mixed}
		 *            arg1 An {@link Ext.Element}, {@link Ext.Component}, or
		 *            an Array of either of these to destroy
		 * @param {Mixed}
		 *            arg2 (optional)
		 * @param {Mixed}
		 *            etc... (optional)
		...
		/**
		 * Attempts to destroy and then remove a set of named properties of the
		 * passed object.
		 * 
		 * @param {Object}
		 *            o The object (most likely a Component) who's properties
		 *            you wish to destroy.
		 * @param {Mixed}
		 *            arg1 The name of the property to destroy and remove from
		 *            the object.
		 * @param {Mixed}
		 *            etc... More property names to destroy and remove.
		 */
		 ...
		 *            arr The Array from which to remove falsy values.
		...
		/**
		 * Creates a copy of the passed Array, filtered to contain only unique
		 * values.
		 * 
		 * @param {Array}
		 ...
		 * 
		 * @param {Array|NodeList}
		 *            arr The Array from which to select the minimum value.
		 * @param {Function}
		 *            comp (optional) a function to perform the comparision
		 *            which determines minimization. If omitted the "<"
		 *            operator will be used. Note: gt = 1; eq = 0; lt = -1
		 ...
		 * 
		 * @param {Array|NodeList}
		 *            arr The Array from which to select the maximum value.
		 * @param {Function}
		 *            comp (optional) a function to perform the comparision
		 *            which determines maximization. If omitted the ">" operator
		 ...
		 * 
		 * @param {Array}
		 *            arr The Array to calculate the mean value of.
		 ...
		 * 
		 * @param {Array}
		 *            arr The Array to calculate the sum value of.
		 ...
		 * 
		 * // Example 2:
		 * Ext.partition(Ext.query(&quot;p&quot;), function(val) {
		 ...
		 * @param {Function}
		 *            truth (optional) a function to determine truth. If this is
		 *            omitted the element itself must be able to be evaluated
		 ...
		 * 
		 * @param {Array|NodeList}
		 *            arr The Array of items to invoke the method on.
		 * @param {String}
		 *            methodName The method name to invoke.
		 * @param {Anything}
		 *            ... Arguments to send into the method invocation.
		 * @return {Array} The results of invoking the method on each item in
		 *         the array.
		 */
		 ...
		 * 
		 * @param {Array|NodeList}
		 *            arr The Array of items to pluck the value from.
		 * @param {String}
		 *            prop The property name to pluck from each element.
		 ...
		 * </code></pre>
		 * 
		 * @param {Arrays|NodeLists}
		 *            arr This argument may be repeated. Array(s) to contribute
		 *            values.
		 * @param {Function}
		 *            zipper (optional) The last item in the argument list. This
		 *            will drive how the items are zipped together.
		 * @return {Array} The zipped set.
		 */
		zip : function() {
			var parts = Ext.partition(arguments, function(val) {
						return !Ext.isFunction(val);
					}), arrs = parts[0], fn = parts[1][0], len = Ext.max(Ext
					.pluck(arrs, "length")), ret = [];
		...
		/**
		 * This is shorthand reference to {@link Ext.ComponentMgr#get}. Looks
		 * up an existing {@link Ext.Component Component} by
		 ...
		 *         found, or <tt>null</tt> if a Class was found.
		 */
		getCmp : function(id) {
		 ...
		 * following values:<div class="mdetail-params">
		 * <ul>
...
Ext.util.TaskRunner = function(interval) {
	 ...
	 *            to execute the task before stopping automatically (defaults to
	 *            indefinite).</div></li>
	 *            <li><code>repeat</code> : Number<div
	 *            class="sub-desc">(optional) The number of times to execute the
...
(function() {
	...
	Ext.lib.Event = function() {
		...
		function _tryPreloadAttach() {
			var ret = false, notAvail = [], element, i, len, v, tryAgain = !loadComplete
					|| (retryCount > 0);
		...
		var pub = {
			...
			getRelatedTarget : function(ev) {
				ev = ev.browserEvent || ev;
				return this
						.resolveTextNode(ev.relatedTarget
								|| (ev.type == MOUSEOUT
										? ev.toElement
										: ev.type == MOUSEOVER
												? ev.fromElement
	...
	Ext.lib.Ajax = function() {
		var activeX = ['MSXML2.XMLHTTP.3.0', 'MSXML2.XMLHTTP',
				'Microsoft.XMLHTTP'], CONTENTTYPE = 'Content-Type';
	...
	(function() {
		...
		Ext.apply(EXTLIB.ColorAnim.prototype, {
			...
			doMethod : function(attr, start, end) {
				var me = this, val, floor = Math.floor, i, len = start.length, v;
