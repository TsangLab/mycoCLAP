...
Ext.tree.ColumnResizer = Ext.extend(Ext.util.Observable, {
			/**
			 * @cfg {Number} minWidth The minimum width the column can be
			 *      dragged to. Defaults to <tt>14</tt>.
			 */
			minWidth : 14,
			...
			handleHdMove : function(e, t) {
				var hw = 5, x = e.getPageX(), hd = e.getTarget(
						'.x-treegrid-hd', 3, true);
				...
				if (hd) {
					...
					if (x - r.left <= hw && hd.dom !== pn.firstChild) {
						...
						while (ps
								&& Ext.fly(ps).hasClass('x-treegrid-hd-hidden')) {
							ps = ps.previousSibling;
						}
						if (ps) {
							this.activeHd = Ext.get(ps);
							ss.cursor = Ext.isWebKit
									? 'e-resize'
									: 'col-resize';
						}
						...
						var ns = hd.dom;
						while (ns
								&& Ext.fly(ns).hasClass('x-treegrid-hd-hidden')) {
			...
			onDrag : function(e) {
				...
				this.proxy.setWidth((cursorX - this.hdX).constrain(
						this.minWidth, this.maxWidth));
			},

			onEnd : function(e) {
				var nw = this.proxy.getWidth(), tree = this.tree;
