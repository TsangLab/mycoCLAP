...
Ext.ux.ValidationStatus = Ext.extend(Ext.Component, {
			/**
			 * @cfg {String} errorIconCls The {@link #iconCls} value to be
			 *      applied to the status message when there is a validation
			 *      error. Defaults to <tt>'x-status-error'</tt>.
			 */
			...
			/**
			 * @cfg {String} validIconCls The {@link #iconCls} value to be
			 *      applied to the status message when the form validates.
			 *      Defaults to <tt>'x-status-valid'</tt>.
			 */
			validIconCls : 'x-status-valid',

			/**
			 * @cfg {String} showText The {@link #text} value to be applied when
			 *      there is a form validation error. Defaults to
			 *      <tt>'The form has errors (click for details...)'</tt>.
			 */
			showText : 'The form has errors (click for details...)',
			/**
			 * @cfg {String} showText The {@link #text} value to display when
			 *      the error list is displayed. Defaults to
			 *      <tt>'Click again to hide the error list'</tt>.
			 */
			hideText : 'Click again to hide the error list',
			/**
			 * @cfg {String} submitText The {@link #text} value to be applied
			 *      when the form is being submitted. Defaults to
			 *      <tt>'Saving...'</tt>.
			 */
			submitText : 'Saving...',
			...
			init : function(sb) {
				sb.on('render', function() {
							...
							this.listAlign = (sb.statusAlign == 'right'
									? 'br-tr?'
									: 'bl-tl?');
							...
							if (this.form) {
								...
								this.form.on('beforeaction',
										function(f, action) {
											if (action.type == 'submit') {
												// Ignore monitoring while
												// submitting otherwise the
												// field validation
												// events cause the status
												// message to reset too early
												this.monitor = false;
											}
										}, this);
								...
								this.form.on('actioncomplete', startMonitor,
										this);
								this.form
										.on('actionfailed', startMonitor, this);
							}
						}, this, {
							single : true
						});
				sb.on({
							...
							afterlayout : {
								...
								fn : function() {
									...
									sb.statusEl.getEl().on('click',
											this.onStatusClick, this, {
												buffer : 200
											});
								}
							},
