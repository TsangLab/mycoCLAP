 ...
 *        <p>
 *        A plugin for Field Components which renders standard Ext form wrapping
 *        and labels round the Field at render time regardless of the layout of
 *        the Container.
 *        </p>
 *        <p>
 *        Usage:
...
Ext.ux.FieldLabeler = (function() {
	...
	// Pulls a named property down from the first ancestor Container it's found
	// in
	...
	return {
		...
		init : function(f) {
			// Replace the Field's onRender method with a sequence that calls
			// the plugin's onRender after the Field's onRender
			f.onRender = f.onRender.createSequence(this.onRender);

			// We need to completely override the onResize method because of the
			// complexity
			f.onResize = this.onResize;

			// Replace the Field's onDestroy method with a sequence that calls
			// the plugin's onDestroy after the Field's onRender
		...
		onRender : function() {
			...
			if (!Ext.isDefined(this.labelSeparator)) {
				this.labelSeparator = getParentProperty.call(this,
						"labelSeparator");
			}
			...
			if (!Ext.isDefined(this.labelAlign)) {
				this.labelAlign = getParentProperty.call(this, "labelAlign")
						|| 'left';
			}
			...
			if (this.labelAlign == 'top') {
				...
				if (!Ext.isDefined(this.labelWidth)) {
					this.labelWidth = getParentProperty
							.call(this, "labelWidth")
							|| 100;
				}
				this.resizeEl.setStyle('padding-left',
						(this.labelWidth + (this.labelPad || 5)) + 'px');
				this.labelWidth += 'px';
			}
		...
		// private
		// Ensure the input field is sized to fit in the content area of the
		// resizeEl (to the right of its padding-left)
		// We perform all necessary sizing here. We do NOT call the current
		// class's onResize because we need this control
