...
Ext.ux.GroupTabPanel = Ext.extend(Ext.TabPanel, {
	...
	onRender : function(ct, position) {
		...
		this.stripWrap = st.createChild({
					...
					cn : {
						tag : 'ul',
						cls : 'x-grouptabs-strip x-grouptabs-tab-strip-'
								+ this.tabPosition
					}
				});
	...
	afterRender : function() {
		...
		if (this.activeGroup !== undefined) {
			var group = (typeof this.activeGroup == 'object')
					? this.activeGroup
	...
	findTargets : function(e) {
		var item = null, itemEl = e.getTarget('li', this.strip);
		if (itemEl) {
	...
	initGroup : function(group, index) {
		...
		var el = before ? this.groupTpl.insertBefore(before, p) : this.groupTpl
				.append(this.strip, p), tl = this.createCorner(el, 'top-'
						+ this.tabPosition), bl = this.createCorner(el,
				'bottom-' + this.tabPosition);
	...
	setActiveGroup : function(group) {
		...
		if (this.activeGroup != group
				&& this.fireEvent('beforegroupchange', this, group,
	...
	onGroupBeforeTabChange : function(group, newTab, oldTab) {
		if (group !== this.activeGroup || newTab !== oldTab) {
			this.strip.select('.x-grouptabs-sub > li.x-grouptabs-strip-active',
					true).removeClass('x-grouptabs-strip-active');
		}
