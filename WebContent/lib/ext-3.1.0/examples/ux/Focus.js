...
(function() {
					...
					if (!this.initialized) {
						...
						this.frameTask = new Ext.util.DelayedTask(function(el) {
									...
									if (newEl != this.curEl) {
										...
										this.sides[0].show().setSize(w,
												this.size).anchorTo(el, 'tl',
												[0, -1]);
										this.sides[2].show().setSize(w,
												this.size).anchorTo(el, 'bl',
												[0, -1]);
										this.sides[1].show().setSize(this.size,
												h).anchorTo(el, 'tr', [-1, 0]);
										this.sides[3].show().setSize(this.size,
												h).anchorTo(el, 'tl', [-1, 0]);
										this.curEl = newEl;
									}
								}, this);
	...
	Ext.a11y.Focusable = Ext.extend(Ext.util.Observable, {
				constructor : function(el, relayTo, noFrame, frameEl) {
					...
					this.addEvents('focus', 'blur', 'left', 'right', 'up',
							'down', 'esc', 'enter', 'space');
				...
				onKeyDown : function(e, t) {
					...
					tf = (t !== this.el.dom)
							? Ext.a11y.FocusMgr.get(t, true)
							: this;
					if (!tf) {
						// this can happen when you are on a focused item within
						// a panel body
						// that is not a Ext.a11y.Focusable
						tf = Ext.a11y.FocusMgr.get(Ext.fly(t)
								.parent('.x-a11y-focusable'));
					}
					...
					if (ret === false
							|| this.fireEvent('keydown', e, t, tf, this) === false) {
						e.stopEvent();
					}
				},
				...
				onTab : function(e, t) {
					...
					if (items && items.length && (i = items.indexOf(t)) !== -1) {
						if (e.shiftKey && i > 0) {
							e.stopEvent();
							items[i - 1].focus();
							Ext.a11y.FocusFrame.frame.defer(20,
									Ext.a11y.FocusFrame, [this.el]);
							return;
	...
	Ext.a11y.FocusMgr = function() {
		...
		return {
			...
			get : function(el, noCreate) {
				return all.get(Ext.id(el))
						|| (noCreate ? false : new Ext.a11y.Focusable(el));
			},
	...
	// we use the new observeClass method to fire our new initFocus method on
	// components
	...
	Ext.override(Ext.Container, {
				...
				onTab : function(e, t, tf) {
					...
					if (rf.component && rf.component !== this) {
						e.stopEvent();
						var item = e.shiftKey ? this
								.getPreviousFocus(rf.component) : this
								.getNextFocus(rf.component);
						item.focus();
					}
				},
				...
				onEsc : function(e, t) {
					...
					if (t === this.el.dom) {
						// "on" the panel, check if this panel has an owner
						// panel and focus that
						// we dont stop the event in this case so that this same
						// check will be
				...
				getFocusItems : function() {
					return this.items && this.items.filterBy(function(o) {
								return o.isFocusable;
							}) || null;
				},

				getEnterItem : function() {
					var ci = this.getFocusItems(), length = ci
							? ci.getCount()
							: 0;

					if (length === 1) {
						return ci.first().getEnterItem
								&& ci.first().getEnterItem() || ci.first();
					} else if (length > 1) {
						return ci.first();
					}
				},
				...
				getNextFocus : function(current) {
					var items = this.getFocusItems(), next = current, i = items
							.indexOf(current), length = items.getCount();
				...
				getPreviousFocus : function(current) {
					var items = this.getFocusItems(), prev = current, i = items
							.indexOf(current), length = items.getCount();
	...
	Ext.override(Ext.Panel, {
		/**
		 * @cfg {Boolean} enableTabbing <tt>true</tt> to enable tabbing.
		...
		getFocusItems : function() {
			...
			if (!items) {
				...
				this.bodyFocus = this.bodyFocus
						|| new Ext.a11y.FocusItem(this.body, this.enableTabbing);
	...
	Ext.override(Ext.TabPanel, {
				...
				onLeft : function(e) {
					...
					e.stopEvent();
					var prev = this.items.itemAt(this.items
				...
				onRight : function(e) {
					...
					e.stopEvent();
					var next = this.items.itemAt(this.items
							.indexOf(this.activeTab)
							+ 1);
					if (next) {
	...
	Ext.override(Ext.tree.TreePanel, {
				...
				afterRender : function() {
					...
					this.bodyFocus = new Ext.a11y.FocusItem(this.body
							.down('.x-tree-root-ct'));
	...
	Ext.override(Ext.Button, {
				...
				initFocus : function() {
					...
					this.fi = this.fi
							|| new Ext.a11y.Focusable(this.btnEl, null, null,
									this.el);
					this.fi.setComponent(this);
	...
	Ext.override(Ext.Toolbar, {
				...
				add : function() {
					var item = Ext.Toolbar.superclass.add
				...
				onFocus : function() {
					...
					if (items && items.getCount() > 0) {
						if (this.lastFocus
								&& items.indexOf(this.lastFocus) !== -1) {
	...
	Ext.override(Ext.menu.BaseItem, {
				initFocus : function() {
					this.fi = new Ext.a11y.Focusable(this, this.parentMenu
									&& this.parentMenu.el || null, true);
				}
			});
	...
	Ext.override(Ext.form.Field, {
				...
				initFocus : function() {
					this.fi = this.fi
							|| new Ext.a11y.Focusable(this, null, true);
	...
	Ext.override(Ext.FormPanel, {
		...
		onTab : function(e, t, tf) {
			if (tf.relayTo.component === this) {
				var item = e.shiftKey
						? this.getPreviousFocus(tf.component)
						: this.getNextFocus(tf.component);
		...
		getNextFocus : function(current) {
			var items = this.getFocusItems(), i = items.indexOf(current), length = items
					.getCount();
