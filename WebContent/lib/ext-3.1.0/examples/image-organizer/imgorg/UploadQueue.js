...
Imgorg.UploadQueue = Ext.extend(Ext.Panel, {
	...
	addAllAlbum : function(btn) {
		if (!this.uploaded) {
			Ext.Msg
					.alert('Warning',
							'You must upload files before you can add them to an Album');
			return;
		}
		var dv = this.getDv();
	...
	tagAll : function(btn) {
		if (!this.uploaded) {
			Ext.Msg.alert('Warning',
					'You must upload files before you can Tag them');
			return;
		}
		var dv = this.getDv();
		var recs = dv.getRecords(dv.getNodes());
	...
	updateProgress : function(swfu, file, complete, total) {
		if (this.progressBars[file.id]) {
			this.progressBars[file.id]
					.updateProgress(file.percentUploaded / 100, Math
									.round(file.percentUploaded)
									+ '% Completed... '
									+ Ext.util.Format
											.fileSize(file.currentSpeed) + 's');
		} else {
...
Ext.ux.SwfuManager = Ext.extend(Ext.util.Observable, {
			constructor : function(config) {
				...
				this.addEvents('filequeued', 'uploadstart', 'uploadprogress',
