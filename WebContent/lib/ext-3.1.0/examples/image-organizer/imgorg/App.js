 ...
 */
/*
 * Sample Image Organizer utilizing Ext.Direct Tagging/Organizing into galleries
 * Image uploading
 */
	...
	SWFUpload.onload = function() {
		var settings = {
			...
			upload_progress_handler : function(file, complete, total) {
				Ext.ux.SwfuMgr.fireEvent('uploadprogress', this, file,
						complete, total);
			},
			upload_error_handler : function(file, error, message) {
				Ext.ux.SwfuMgr.fireEvent('uploaderror', this, file, error,
						message);
			},
			...
			post_params : {
				...
				// needs extTID – Transaction ID to associate with this
				// request.
			}
		};
		...
		getTabs : function() {
			...
			if (this.debugSWF) {
				tabs.push({
							...
							listeners : {
								render : function() {
									Ext.fly('SWFUpload_Console').applyStyles({
												height : '100%',
												width : '100%'
											});
								}
