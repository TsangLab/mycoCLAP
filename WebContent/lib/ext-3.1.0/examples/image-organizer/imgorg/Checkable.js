...
Ext.ux.MultiCombo.Checkable.prototype = {
	...
	/**
	 * @cfg {String} itemSelector The itemSelector used with Combo's internal
	 *      DataView [x-combo-list]
	 */
	...
	/**
	 * @cfg {String} separatorHtml arbitrary html rendered after Checkable
	 *      controls. Can be used to add an html separator markup.
	 */
	separatorHtml : '',
	...
	init : function(combo) {
		...
		if (!combo.tpl) {
			combo.tpl = [
					'<tpl for=".">',
					'<div class="' + cls + '-item">{' + combo.displayField
							+ '}</div>', '</tpl>'].join('');
		}
		combo.tpl = [
				...
				'<input id="' + id + '" type="checkbox" />&nbsp;<label>'
						+ this.selectAllText + '</label>',
				'</span>',
				'&nbsp;<span class="' + this.cls + '-clear-all">(<a href="#">'
						+ this.clearAllText + '</a>)</span>', '</div>',
				this.separatorHtml].join('')
				+ combo.tpl
						.replace(
								new RegExp('({' + combo.displayField + '})'),
								"<input type=\"checkbox\" <tpl if=\"values._checked\">checked</tpl> />&nbsp;<label>$1</label>");
	...
	getCheckbox : function() {
		return this.combo.view.el.child('.' + this.cls + ' '
	...
	onSelectionChange : function(dv, rs) {
		this.checked = (rs.length > 0 && rs.length == this.combo.store
				.getCount()) ? true : false;
		this.getCheckbox().checked = this.checked;

		var selector = this.checkSelector;
		setTimeout(function() {
					dv.el.select(dv.itemSelector + ' ' + selector).each(
							function(f) {
								f.dom.checked = false;
							});
					dv.el.select('.' + dv.selectedClass + ' ' + selector).each(
							function(f) {
								f.dom.checked = true;
							});
				}, 1);
	...
	/**
	 * selectNext Called by MultiCombo. use this to apply hover-class to this
	 * row.
	 * 
	 * @param {Object}
	 *            sender
