...
(function		() {
			...
			onActivate : function() {
				...
				if (p) {
					sliderValue = Ext.num(p.get('sliderValue'), 0);
					var slider = this.getBottomToolbar()
							.getComponent('size-slider');
			...
			onChange : function(slider, e) {
				...
				Ext.util.CSS.updateRule('.images-view .thumb img', 'height',
						this.minWidth + slider.getValue() * 3);
			},
...
Ext.extend(ImageDragZone, Ext.dd.DragZone, {
			...
			getDragData : function(e) {
				...
				if (target) {
					...
					if (selNodes.length == 1) {
						...
						var count = document.createElement('div'); // selected
																	// image
																	// count
			...
			// to get the new tree node (there are also other way, but this is
			// easiest)
