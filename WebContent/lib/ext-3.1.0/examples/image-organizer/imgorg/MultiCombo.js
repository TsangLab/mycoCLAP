...
Ext.ux.MultiCombo = Ext.extend(Ext.form.ComboBox, {
	...
	/**
	 * @cfg {String} highlightClass The css class applied to rows which are
	 *      hovered with mouse selected via key-nav, or highlighted when a
	 *      text-query matches a single item.
	 */
	highlightClass : 'x-grid3-row-over',
	/**
	 * @cfg {Number} autoSelectKey [44] COMMA Sets the key used to auto-select
	 *      an auto-suggest highlighted query. When pressed, the highlighted
	 *      text-item will be selected as if the user selected the row with a
	 *      mouse click.
	 */
	autoSelectKey : 44,
	/**
	...
	/**
	 * @cfg {Number} maxDisplayRows The maximum number of rows to show before
	 *      applying vscroll
	 */
	maxDisplayRows : null,
	...
	initComponent : function() {
		...
		// when blurring out of field, ensure that rawValue contains ONLY items
		// contained in Store.
		this.on('blur', this.validateSelections.createDelegate(this));

		// create an auto-select key handler, like *nix-based console [tab] key
		// behaviour
		...
		this.addEvents(
				/**
				 * @event initview Fires when Combo#initView is called. gives
				 *        plugins a chance to interact with DataView
				 * @author Chris Scott
				 * @param {Combo}
				 *            this
				 * @param {DataView}
	...
	onTriggerClick : function() {
		...
		if (this.isExpanded()) {
			...
			if (this.triggerAction == 'all') {
				...
				var vlen = this.getValue().length, slen = this.view
			...
			if (this.lastQuery !== q) {
				if (typeof(this.lastQuery) != 'undefined') {
					if (q.match(new RegExp('^' + this.allSelectedText))) {
						...
						this.query = this.store.data.filterBy(this.store
								.createFilterFn(this.displayField, new RegExp(
												'^' + items.join('$|^') + '$',
												"i"), false, false));
					} else {
						this.query = null;
				...
				if (this.mode == 'local') {
					...
					var last = items.pop();
					this.matches = this.store.data
							.filterBy(this.store.createFilterFn(
									this.displayField, new RegExp('^' + last,
											"i"), false, false)).filterBy(this
			...
			if (this.lastQuery == this.allQuery) {
				...
				if (this.matches != null) {
					if (this.matches.getCount() == 1) {
						this
								.highlight(this.store.indexOf(this.matches
										.first()));
						this.scrollIntoView();
					}
				} else {
					// @HACK: If store was configured with a proxy, set its mode
					// to local now that its populated with data.
					// Re-execute the query now.
					this.mode = 'local';
					this.lastQuery = undefined;
					this.doQuery(this.getRawValue(), true);
				}
				if (this.typeAhead && this.lastKey != Ext.EventObject.DOWN
	...
	onSelect : function(record, index) {
		...
		if (this.fireEvent('beforeselect', this, record, index) !== false) {
			...
			this.updateValue(value, (value.length != this.store.getCount())
							? text.join(', ')
	...
	onTypeAhead : function() {
		if (this.store.getCount() > 0) {
			...
			var rs = this.store.data.filterBy(this.store.createFilterFn(
					this.displayField, new RegExp(query, "i"), false, false))
					.filterBy(this.createTypeAheadFilterFn(items));
			...
			if (rs.getCount() == 1) {
				...
				if (!this.view.isSelected(rindex)) {
					...
					if (selStart != len) {
						...
						this.selectText(selStart, (selEnd != null)
										? selEnd
	...
	getCaretPosition : function(raw) {
		...
		if (document.selection) { // <-- IE, ugh:
			...
			// Save the current value. We will need this value later to find
			// out, where the text has been changed
			...
			/*
			 * Find the first occurance, where the original differs from the
			 * actual content. This could be the startposition of our text
			 * selection, but it has not to be. Think of the selection "ab" and
			 * replacing it with "ac". The first difference would be the "c",
			 * while the start position is the "a"
			 */
			...
			/*
			 * To get the real start position, we iterate through the string
			 * searching for the whole replacement text - "abc", as long as the
			 * first difference is not reached. If you do not understand that
			 * logic - no blame to you, just copy & paste it ;)
			 */
			for (var index = 0, start = 0; tmp.match(text)
					&& (tmp = tmp.replace(text, "")) && index <= diff; index = start
					+ text.length) {
	...
	// e.g.: if store contains: "betty, barney, bart" and betty is already
	// selected,
	// when user types "b", only "bart" and "barney" should be returned as
	// possible matches,
	...
	/**
	 * setValue Accepts a comma-separated list of ids or an array. if given a
	 * string, will conver to Array.
	 * 
	 * @param {Array,
	 *            String} v
	 */
	setValue : function(v) {
		...
		if (typeof(v) == 'string') { // <-- "1,2,3"
			value = v.match(/\d+/g); // <-- strip multiple spaces and split
										// on ","
			if (value) {
	...
	getTextValue : function() {
		if (this.value.length == this.store.getCount()) {
			...
			var text = [];
			this.store.data.filterBy(this.store.createFilterFn(this.valueField,
					new RegExp(this.value.join('|'), "i"), false, false)).each(
					function(r) {
	...
	select : function(index, scrollIntoView) {
		if (!typeof(index) == 'number') {
			throw new Error('MultiCombo#select expected @param {Number} index but got: '
					+ typeof(index));
		}
		this.view.isSelected(index)
	...
	selectByValue : function(v, scrollIntoView) {
		if (v.length) {
			var indexes = [];
			var rs = this.store.data.filterBy(this.store.createFilterFn(
					this.valueField, new RegExp(v.join('|'), "i"))).each(
					function(r) {
						indexes.push(this.store.indexOf(r));
					}, this);
			if (indexes.length) {
		...
		this.keyNav = new Ext.KeyNav(this.el, {
					...
					"enter" : function(e) {
						...
						if (this.inKeyMode === true) {
							...
							var word = '';
							if (pos !== false && pos < raw.length) {
								word = Ext.util.Format.trim(raw.substr(0, pos)
										.split(',').pop());
							} else {
								word = Ext.util.Format.trim(raw.split(',')
										.pop());
							}
							/** **************************************************** */
					...
					"tab" : function(e) {
						if (this.matches != null
								&& this.matches.getCount() == 1) {
							var idx = this.store.indexOf(this.matches.first());
							if (!this.view.isSelected(idx)) {
								this.select(this.store.indexOf(this.matches
												.first()), true);
							}
						} else if (this.value.length == 0
								&& this.getRawValue().length > 0) {
							this.setRawValue('');
						}
						this.collapse();
						return true;
	...
	// private, blur-handler to ensure that rawValue contains only values from
	// selections, in the same order as selected
	...
	highlight : function(index) {
		this.view.el.select('.' + this.highlightClass)
	...
	getHighlightedIndex : function() {
		...
		return (node)
				? this.store.indexOf(this.view.getRecord(node))
				: this.highlightIndex;
	},
