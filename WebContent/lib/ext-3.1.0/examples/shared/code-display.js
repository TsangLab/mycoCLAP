 ...
 */
// Create and append to the body, a Panel containing a block of code from the
// passed URL
				...
				listeners : {
					render : function(p) {
						p.getUpdater().setRenderer({
							render : Ext.isIE ? function(el, response, scripts,
									callback) {
								el.update('');
...
// Patch to allow XHR to local files. From hendricd:
// http://extjs.com/forum/member.php?u=8730
Ext.apply(Ext.lib.Ajax, {
	...
	handleTransactionResponse : function(o, callback, isAbort) {
		...
		if (o.status.isError) { /*
								 * checked again in case exception was raised -
								 * ActiveX was disabled during XML-DOM creation?
								 */
			// And mixin everything the XHR object had to offer as well
			responseObject = Ext.applyIf(responseObject || {}, this
							.createExceptionObject(o.tId, callback.argument,
	...
	createResponseObject : function(o, callbackArg) {
		...
		if (o.status.isLocal) {
			...
			o.status.isOK = ((o.status.status = (!!obj.responseText.length)
					? 200
					: 404) == 200);
			...
			if (o.status.isOK
				...
				try { // ActiveX may be disabled
					if (typeof(DOMParser) == 'undefined') {
						...
						try { // Opera 9 will fail parsing non-XML content, so
								// trap here.
							var domParser = new DOMParser();
							xdoc = domParser.parseFromString(obj.responseText,
									'application\/xml');
						} catch (ex) {
						} finally {
							domParser = null;
			...
			if (obj.responseXML) {
				...
				var parseBad = (obj.responseXML.parseError || 0) != 0
		...
		if (!o || o.status.isError) {
			...
			if (this.useDefaultXhrHeader) {
				if (!this.defaultHeaders['X-Requested-With']) {
					this.initHeader('X-Requested-With', this.defaultXhrHeader,
							true);
				}
			}
	...
	makeRequest : function(method, uri, callback, postData, options) {
		...
		if (!o || o.status.isError) {
			...
			o.options = options;
			try {
				o.conn.open(method, uri, options.async, options.userId,
						options.password);
			} catch (ex) {
				o.status.isError = true;
				o.status.error = ex;
			...
			if (o.options.async) { // Timers won't work here as it's a blocking
									// call
				this.handleReadyState(o, callback);
			}
