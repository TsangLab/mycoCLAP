	...
	function doBlur() {
		if (editing && !focused) {
			...
			if (!Ext.isEmpty(title)) {
				...
				ntTitle.setValue('');
				if (userTriggered) { // if the entered to add the task, then
...
function loadDemoTasks(store) {
	...
	store.addTask({
				taskId : Task.nextId(),
				title : 'Start documentation of Ext 2.0',
				category : 'Ext',
				description : '',
				dueDate : s.add('d', 21),
				completed : false
	...
	store.addTask({
				taskId : Task.nextId(),
				title : 'Finish task list demo app',
				category : 'Ext',
				description : '',
				dueDate : s.add('d', 2),
				completed : false
