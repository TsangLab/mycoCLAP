...
Ext.onReady(function() {
	...
	new Ext.Panel({
				...
				items : [{
					...
					id : 'basic-button',
					text : 'Do Loading',
					handler : loadFn.createCallback('basic-button',
							'basic-statusbar')
				}],
	...
	new Ext.Panel({
				...
				items : [{
					...
					id : 'right-button',
					text : 'Do Loading',
					handler : loadFn.createCallback('right-button',
							'right-statusbar')
				}],
				bbar : new Ext.ux.StatusBar({
	...
	var win = new Ext.Window({
				...
				items : [{
					...
					id : 'win-button',
					text : 'Do Loading',
					handler : loadFn.createCallback('win-button',
							'win-statusbar')
				}],
				bbar : new Ext.ux.StatusBar({
							id : 'win-statusbar',
	...
	new Ext.Panel({
				...
				items : {
					...
					growMax : 200,
					listeners : {
						// After each keypress update the word and character
						// count text items
						'keypress' : {
							fn : function(t) {
								var v = t.getValue(), wc = 0, cc = v.length
										? v.length
										: 0;
								...
								Ext.fly(wordCount.getEl()).update('Words: '
										+ wc);
								Ext.fly(charCount.getEl()).update('Chars: '
										+ cc);
							},
							buffer : 1
							// buffer to allow the value to update first
						}
					}
				},
				listeners : {
