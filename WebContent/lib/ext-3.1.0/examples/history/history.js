...
Ext.onReady(function() {
			...
			// The only requirement for this to work is that you must have a
			// hidden field and
			...
			// Needed if you want to handle history for multiple components in
			// the same page.
			...
			var tp = new Ext.TabPanel({
						...
						items : [{
							...
							listeners : {
								'tabchange' : function(tabPanel, tab) {
									Ext.History.add(tabPanel.id
											+ tokenDelimiter + tab.id);
								}
							}
						}, {
							title : 'Tab 2',
						...
						listeners : {
							'tabchange' : function(tabPanel, tab) {
								// Ignore tab1 since it is a separate tab panel
								// and we're managing history for it also.
								// We'll use its handler instead in that case so
								// we don't get duplicate nav events for sub
								// tabs.
								if (tab.id != 'tab1') {
									Ext.History.add(tabPanel.id
											+ tokenDelimiter + tab.id);
			...
			Ext.History.on('change', function(token) {
						if (token) {
							...
							// This is the initial default state. Necessary if
							// you navigate starting from the
