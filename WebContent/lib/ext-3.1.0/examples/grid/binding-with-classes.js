 ...
 * @cfg {String} url This will be a url of a location to load the BookStore This
 *      is a specialized Store which maintains books. It already knows about
 *      Amazon's XML definition and will expose the following Record defintion: -
 *      Author - Manufacturer - ProductGroup - DetailPageURL
 */
	...
	Ext.applyIf(config, {
				reader : new Ext.data.XmlReader({
							...
							mapping : 'ItemAttributes > Author'
								...
								// Detail URL is not part of the column model of
								// the grid
								'DetailPageURL'])
			});
 ...
 * App.BookGrid
 * 
 * @extends Ext.grid.GridPanel This is a custom grid which will display book
 *          information. It is tied to a specific record definition by the
 *          dataIndex properties.
 * 
 * It follows a very custom pattern used only when extending Ext.Components in
 * which you can omit the constructor.
 * 
...
App.BookGrid = Ext.extend(Ext.grid.GridPanel, {
			...
			initComponent : function() {
				Ext.apply(this, {
							...
							// Note that the DetailPageURL was defined in the
							// record definition but is not used
							// here. That is okay.
							columns : [{
										header : "Author",
										width : 120,
							...
							// Note the use of a storeId, this will register
							// thisStore
							// with the StoreMgr and allow us to retrieve it
							// very easily.
							store : new App.BookStore({
										storeId : 'gridBookStore',
										url : 'sheldon.xml'
									}),
							// force the grid to fit the space which is
							// available
							viewConfig : {
								forceFit : true
							}
 ...
 * 
 * This demonstrates adding 2 custom properties (tplMarkup and startingMarkup)
 * to the class. It also overrides the initComponent method and adds a new
 * method called updateDetail.
 * 
 ...
 * @extends Ext.Panel
 * 
 * This is a specialized panel which is composed of both a bookgrid and a
 * bookdetail panel. It provides the glue between the two components to allow
 * them to communicate. You could consider this the actual application.
 * 
 */
App.BookMasterDetail = Ext.extend(Ext.Panel, {
			...
			initEvents : function() {
				...
				var bookGridSm = this.getComponent('gridPanel')
						.getSelectionModel();
			...
			onRowSelect : function(sm, rowIdx, r) {
				// getComponent will retrieve itemId's or id's. Note that
				// itemId's
