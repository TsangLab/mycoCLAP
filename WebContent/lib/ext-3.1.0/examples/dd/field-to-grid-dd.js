...
Ext.ux.CellFieldDropZone = Ext.extend(Ext.dd.DropZone, {
			...
			init : function(grid) {
				if (grid.rendered) {
					...
					Ext.ux.CellFieldDropZone.superclass.constructor.call(this,
							this.view.scroller);
				} else {
			...
			getTargetFromEvent : function(e) {
				...
				if (t) {
					...
					// We *are* within a grid cell, so ask the View exactly
					// which one,
					// Extract data from the Model to create a target object for
					// processing in subsequent onNodeXXXX methods. Note that
					// the target does
					// not have to be a DOM element. It can be whatever the
					// noNodeXXX methods are
					...
					if ((rowIndex !== false) && (columnIndex !== false)) {
						return {
							...
							fieldName : this.grid.getColumnModel()
			...
			onNodeEnter : function(target, dd, e, dragData) {
				...
				// Check whether the data type of the column being dropped on
				// accepts the
				// dragged field type. If so, set dropOK flag, and highlight the
				// target node.
			...
			// Return the class name to add to the drag proxy. This provides a
			// visual indication
			...
			// Process the drop event if we have previously ascertained that a
			// drop is OK.
			onNodeDrop : function(target, dd, e, dragData) {
				if (this.dropOK) {
					target.record.set(target.fieldName, dragData.field
									.getValue());
					return true;
				}
			}
...
Ext.ux.PanelFieldDragZone = Ext.extend(Ext.dd.DragZone, {
			...
			// Call the DRagZone's constructor. The Panel must have been
			// rendered.
			init : function(panel) {
				if (panel.nodeType) {
					Ext.ux.PanelFieldDragZone.superclass.init.apply(this,
							arguments);
				} else {
					if (panel.rendered) {
						Ext.ux.PanelFieldDragZone.superclass.constructor.call(
								this, panel.getEl());
						var i = Ext.fly(panel.getEl()).select('input');
						i.unselectable();
					} else {
						panel.on('afterlayout', this.init, this, {
									single : true
								});
					}
				}
			...
			// On mousedown, we ascertain whether it is on one of our draggable
			// Fields.
			// If so, we collect data about the draggable object, and return a
			// drag data
			// object which contains our own data, plus a "ddel" property which
			// is a DOM
			...
			getDragData : function(e) {
				...
				if (t) {
					...
					// Ugly code to "detach" the drag gesture from the input
					// field.
					// Without this, Opera never changes the mouseover target
					// from the input field
					// even when dragging outside of the field - it just keeps
					// selecting.
					if (Ext.isOpera) {
