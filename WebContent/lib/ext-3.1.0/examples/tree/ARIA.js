...
(function() {
	...
	Ext.override(Ext.tree.TreePanel, {
		initARIA : function() {
			...
			this.getSelectionModel().on('selectionchange', this.onNodeSelect,
					this);
		...
		onNodeRender : function(node) {
			var a = node.ui.anchor, level = this.rootVisible ? 1 : 0, pnode = node;

			if (node.isRoot) {
				ARIA.setRole(this.ariaTreeEl, 'tree');
				ARIA.setProperty(this.ariaTreeEl, 'aria-labelledby', Ext
								.id(node.ui.textNode));
				ARIA.setProperty(this.ariaTreeEl, 'aria-activedescendant',
						'false');
				if (!this.rootVisible) {
					return;
				}
			...
			ARIA.setProperty(node.ui.wrap, 'aria-labelledby', Ext
							.id(node.ui.textNode));
			...
			if (!node.isLeaf()) {
				...
				ARIA.setProperty(node.ui.wrap, 'aria-expanded', node
								.isExpanded());
			}
		},

		onNodeSelect : function(sm, node) {
			ARIA.setProperty(this.ariaTreeEl, 'aria-activedescendant', Ext
							.id(node.ui.wrap));
