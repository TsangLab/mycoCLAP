...
Ext.debug.DomTree = Ext.extend(Ext.tree.TreePanel, {
			...
			initComponent : function() {
				...
				function findNode(n) {
					...
					cn.select();
					var a = cn.ui.anchor;
					this.getTreeEl().dom.scrollTop = Math.max(0, a.offsetTop
									- 10);
					// treeEl.dom.scrollLeft = Math.max(0 ,a.offsetLeft-10); no
					// likey
					cn.highlight();
					return true;
				}
				...
				/*
				 * function onNodeSelect(t, n, last){ return; if(last &&
				 * last.unframe){ last.unframe(); } var props = {}; if(n &&
				 * n.htmlNode){ if(frameEl.pressed){ n.frame(); }
				 * if(inspecting){ return; } addStyle.enable();
				 * reload.setDisabled(n.leaf); var dom = n.htmlNode;
				 * stylePanel.setTitle(nodeTitle(dom)); if(styles &&
				 * !showAll.pressed){ var s = dom.style ? dom.style.cssText :
				 * ''; if(s){ var m; while ((m = styleRe.exec(s)) != null){
				 * props[m[1].toLowerCase()] = m[2]; } } }else if(styles){ var
				 * cl = Ext.debug.cssList; var s = dom.style, fly =
				 * Ext.fly(dom); if(s){ for(var i = 0, len = cl.length; i<len;
				 * i++){ var st = cl[i]; var v = s[st] || fly.getStyle(st); if(v !=
				 * undefined && v !== null && v !== ''){ props[st] = v; } } }
				 * }else{ for(var a in dom){ var v = dom[a]; if((isNaN(a+10)) &&
				 * v != undefined && v !== null && v !== '' && !(Ext.isGecko &&
				 * a[0] == a[0].toUpperCase())){ props[a] = v; } } } }else{
				 * if(inspecting){ return; } addStyle.disable();
				 * reload.disabled(); } stylesGrid.setSource(props);
				 * stylesGrid.treeNode = n; stylesGrid.view.fitColumns(); }
				 */
				...
				// tree.getSelectionModel().on('selectionchange', onNodeSelect,
				// null, {buffer:250});
...
Ext.debug.ComponentNodeUI = Ext.extend(Ext.tree.TreeNodeUI, {
			onOver : function(e) {
				...
				if (cmp.el && cmp.el.mask && cmp.id != 'x-debug-browser') {
					try { // Oddly bombs on some elements in IE, gets any we
							// care about though
						cmp.el.mask();
					} catch (e) {
					}
...
Ext.debug.ComponentInspector = Ext.extend(Ext.tree.TreePanel, {
	...
	refresh : function() {
		...
		if (ci) {
			ci.message('refreshed component tree - '
					+ Ext.ComponentMgr.all.length)
		}
	}
});
					...
					listeners : {
						check : function(cb, val) {
							if (val) {
								...
								Ext.ComponentMgr.all.on('remove',
										this.onRemove, this);
							} else {
								Ext.ComponentMgr.all
										.un('add', this.onAdd, this);
								Ext.ComponentMgr.all.un('remove',
										this.onRemove, this);
							}
						},
						scope : this
...
Ext.debug.ObjectInspector = Ext.extend(Ext.tree.TreePanel, {
			...
			onRender : function() {
				Ext.debug.ObjectInspector.superclass.onRender.apply(this,
						arguments);
