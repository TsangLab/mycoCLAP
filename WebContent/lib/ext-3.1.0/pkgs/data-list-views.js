...
Ext.list.ListView = Ext.extend(Ext.DataView, {
	...
	initComponent : function() {
		...
		for (var f = 0; f < l; f++) {
			var m = k[f];
			if (!m.isColumn) {
				m.xtype = m.xtype ? (/^lv/.test(m.xtype) ? m.xtype : "lv"
						+ m.xtype) : "lvcolumn";
				m = Ext.create(m)
			}
			if (m.width) {
	...
	onResize : function(b, d) {
		...
		if (Ext.isNumber(d)) {
			c.style.height = (d - f.parentNode.offsetHeight) + "px"
		}
	},
...
Ext.list.ColumnResizer = Ext.extend(Ext.util.Observable, {
	...
	handleHdMove : function(i, f) {
		var b = 5, a = i.getPageX(), h = i.getTarget("em", 3, true);
		if (h) {
			var g = h.getRegion(), d = h.dom.style, c = h.dom.parentNode;
			if (a - g.left <= b && c != c.parentNode.firstChild) {
	...
	onEnd : function(k) {
		var h = this.proxy.getWidth();
		this.proxy.remove();
		var g = this.hdIndex, n = this.view, f = n.columns, j = f.length, p = this.view.innerHd
				.getWidth(), d = this.minPct * 100, q = Math
				.ceil((h * n.maxWidth) / p), o = (f[g].width * 100) - q, m = Math
				.floor(o / (j - 1 - g)), l = o - (m * (j - 1 - g));
		for (var c = g + 1; c < j; c++) {
			var b = (f[c].width * 100) + m, a = Math.max(d, b);
			if (b != a) {
				l += b - a
			}
			f[c].width = a / 100
