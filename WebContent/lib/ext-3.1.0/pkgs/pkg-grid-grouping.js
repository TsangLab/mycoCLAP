...
Ext.grid.GroupingView = Ext.extend(Ext.grid.GridView, {
	...
	processEvent : function(b, g) {
		var f = g.getTarget(".x-grid-group-hd", this.mainBody);
		if (f) {
			var d = this.getGroupField(), c = this.getPrefix(d), a = f.id
					.substring(c.length);
			a = a.substr(0, a.length - 3);
			if (a) {
				this.grid.fireEvent("group" + b, this.grid, d, a, g)
			}
		}
	},
	...
	doRender : function(c, f, p, a, o, q) {
		...
		var y = this.getGroupField(), n = this.cm.findColumnIndex(y), v;
		this.enableGrouping = (this.enableGrouping === false) ? false : !!y;
		if (!this.enableGrouping || this.isUpdating) {
			return Ext.grid.GroupingView.superclass.doRender.apply(this,
					arguments)
		}
		var h = "width:" + this.getTotalWidth() + ";", e = this.cm.config[n], b = e.groupRenderer
				|| e.renderer, s = this.showGroupName
		...
		for (t = 0, u = f.length; t < u; t++) {
			...
			if (!k || k.group != v) {
				...
				this.state[m] = !(Ext.isDefined(this.state[m])
						? !this.state[m]
						: this.startCollapsed);
				k = {
					group : v,
	...
	constructId : function(c, e, a) {
		var b = this.cm.config[a], d = b.groupRenderer || b.renderer, f = (this.groupMode == "value")
				? c
				: this.getGroup(c, {
							data : {}
						}, d, 0, a, this.ds);
