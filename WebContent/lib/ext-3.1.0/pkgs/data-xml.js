...
Ext.extend(Ext.data.XmlReader, Ext.data.DataReader, {
	...
	readResponse : function(e, a) {
		var d = Ext.DomQuery, f = a.responseXML;
		var b = new Ext.data.Response({
					action : e,
					success : this.getSuccess(f),
					message : this.getMessage(f),
					data : this.extractData(d.select(this.meta.record, f)
									|| d.select(this.meta.root, f), false),
					raw : f
				});
	...
	buildExtractors : function() {
		...
		this.getRoot = function(f) {
			return (!Ext.isEmpty(f[this.meta.record]))
					? f[this.meta.record]
					: f[this.meta.root]
		};
