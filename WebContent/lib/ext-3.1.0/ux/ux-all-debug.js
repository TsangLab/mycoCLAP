...
Ext.ux.grid.GridFilters = Ext.extend(Ext.util.Observable, {
    ...
    init : function (grid) {
        if (grid instanceof Ext.grid.GridPanel) {
            ...
            // assumes no filters were passed in the constructor, so try and use
			// ones from the colModel
    ...
    onMenu : function (filterMenu) {
        ...
        if (filter) {
/*
 * TODO: lazy rendering if (!filter.menu) { filter.menu = filter.createMenu(); }
 */
	 ...
	 *            filters Either an Array of filter configuration objects or an
	 *            Ext.grid.ColumnModel. The columns of a passed
	 *            Ext.grid.ColumnModel will be examined for a
	 *            <code>filter</code> property and, if present, will be used
	 *            as the filter configuration object.
	 */
...
Ext.ux.grid.filter.Filter = Ext.extend(Ext.util.Observable, {
	 ...
	 */
    /**
	 * @cfg {String} dataIndex The {@link Ext.data.Store} dataIndex of the field
	 *      this filter represents. The dataIndex does not actually have to
	 *      exist in the store.
	 */
    dataIndex : null,
    /**
    ...
    menu : null,
    /**
	 * @cfg {Number} updateBuffer Number of milliseconds to wait after user
	 *      interaction to fire an update. Only supported by filters: 'list',
	 *      'numeric', and 'string'. Defaults to 500.
	 */
    updateBuffer : 500,
    ...
    constructor : function (config) {
        ...
        this.addEvents(
            ...
            'deactivate',
            /**
			 * @event serialize Fires after the serialization process. Use this
			 *        to attach additional parameters to serialization data
			 *        before it is encoded and sent to the server.
			 * @param {Array/Object}
			 *            data A map or collection of maps representing the
    ...
    /**
	 * Destroys this filter by purging any event listeners, and removing any
	 * menus.
	 */
    destroy : function(){
        if (this.menu){
    ...
    /**
	 * Template method to be implemented by all subclasses that is to initialize
	 * the filter and install required menu items. Defaults to Ext.emptyFn.
	 */
    init : Ext.emptyFn,
    
    /**
	 * Template method to be implemented by all subclasses that is to get and
	 * return the value of the filter. Defaults to Ext.emptyFn.
	 * 
	 ...
	 * value of the filter and fire the 'update' event. Defaults to Ext.emptyFn.
	 * 
	 * @param {Object}
	 ...
	 * <tt>true</tt> if the filter has enough configuration information to be
	 * activated. Defaults to <tt>return true</tt>.
	 * 
	 * @return {Boolean}
	 */
    ...
    /**
	 * Template method to be implemented by all subclasses that is to get and
	 * return serialized filter data for transmission to the server. Defaults to
	 * Ext.emptyFn.
	 */
	 ...
	 * the provided Ext.data.Record against the filters configuration. Defaults
	 * to <tt>return true</tt>.
	 * 
    ...
    /**
	 * Returns the serialized filter data for transmission to the server and
	 ...
	 * @extends Ext.ux.grid.filter.Filter Boolean filters use unique radio group
...
Ext.ux.grid.filter.BooleanFilter = Ext.extend(Ext.ux.grid.filter.Filter, {
	 ...
	 *      option to be checked by default. Defaults to false.
	 */
	defaultValue : false,
	/**
...
Ext.ux.grid.filter.DateFilter = Ext.extend(Ext.ux.grid.filter.Filter, {
	 ...
	 *          shown. ListFilter utilizes the {@link Ext.ux.menu.ListMenu}
	 *          component.
	 *          </p>
	 *          <p>
	 *          Although not shown here, this class accepts all configuration
	 *          options for {@link Ext.ux.menu.ListMenu}.
	 *          </p>
	 * 
	 * <p>
	 * <b><u>Example Usage:</u></b>
...
Ext.ux.grid.filter.ListFilter = Ext.extend(Ext.ux.grid.filter.Filter, {
	 ...
	 * @cfg {Array} options
	 *      <p>
	 *      <code>data</code> to be used to implicitly create a data store to
	 *      back this list when the data source is <b>local</b>. If the data
	 *      for the list is remote, use the <code>{@link #store}</code> config
	 *      instead.
	 *      </p>
	 *      <br>
	 *      <p>
    ...
    phpMode : false,
    /**
	 * @cfg {Ext.data.Store} store The {@link Ext.data.Store} this list should
	 *      use as its data source when the data source is <b>remote</b>. If
	 *      the data for the list is local, use the
...
Ext.ux.menu.ListMenu = Ext.extend(Ext.menu.Menu, {
    ...
    constructor : function (cfg) {
        ...
        this.addEvents(
            /**
			 * @event checkchange Fires when there is a change in checked items
			 *        from this list
			 * @param {Object}
 ...
 * @extends Ext.menu.Menu Custom implementation of Ext.menu.Menu that has
 *          preconfigured items for gt, lt, eq.
 *          <p>
 *          <b><u>Example Usage:</u></b>
 *          </p>
 * 
...
Ext.reg('grouptabpanel', Ext.ux.GroupTabPanel);/*
												 * Note that this control will
												 * most likely remain as an
												 * example, and not as a core
												 * Ext form control. However,
												 * the API will be changing in a
												 * future release and so should
												 * not yet be treated as a
												 * final, stable API at this
												 * time.
												 */
...
Ext.ux.data.PagingMemoryProxy = Ext.extend(Ext.data.MemoryProxy, {
    ...
    doRequest : function(action, rs, params, reader, callback, scope, options){
        ...
        if (params.sort !== undefined) {
            // use integer as params.sort to specify column, since arrays are
			// not named
...
Ext.ux.ProgressBarPager  = Ext.extend(Object, {
	...
	defaultText    : 'Loading...',
		 ...
		 *      <p>
		 *      A {@link Ext.Fx Ext.Fx} configuration object. Default is {
		 *      duration : 1, easing : 'bounceOut' }.
		 *      </p>
		 */
	defaultAnimCfg : {
		duration   : 1,
...
Ext.ux.grid.RowEditor = Ext.extend(Ext.Panel, {
    ...
    initComponent: function(){
        ...
        this.addEvents(
            /**
			 * @event beforeedit Fired before the row editor is activated. If
			 *        the listener returns <tt>false</tt> the editor will not
			 *        be activated.
			 ...
			 *            roweditor This object
			 * @param {Boolean}
			 *            forced True if the cancel button is pressed, false is
			 *            the editor was invalid.
			 */
            'canceledit',
            /**
			 ...
			 *        validation. This event is fired after the store's update
			 *        event is fired with this edit.
			 * @param {Ext.ux.grid.RowEditor}
			 *            roweditor This object
			 * @param {Object}
			 *            changes Object with changes made to the record.
...
// backwards compat
Ext.grid.RowExpander = Ext.ux.grid.RowExpander;// We are adding these custom
												// layouts to a namespace that
												// does not
// exist by default in Ext, so we have to add the namespace first:
Ext.ns('Ext.ux.layout');
 ...
 *          automatically set {@link #editable} to false and call
 *          {@link Ext.Element#unselectable} on inner elements. Re-enabling
 *          editable after calling this will NOT work.
 *          </p>
...
Ext.Spotlight = Ext.ux.Spotlight;/**
									 ...
									 *        <p>
									 *        Basic status bar component that
									 *        can be used as the bottom toolbar
									 *        of any {@link Ext.Panel}. In
									 *        addition to supporting the
									 *        standard {@link Ext.Toolbar}
									 *        interface for adding buttons,
									 *        menus and other items, the
									 *        StatusBar provides a greedy status
									 *        element that can be aligned to
									 *        either side and has convenient
									 *        methods for setting the status
									 *        text and icon. You can also
...
Ext.ux.StatusBar = Ext.extend(Ext.Toolbar, {
	 ...
	 */
    /**
	 * @cfg {String} iconCls A CSS class that will be <b>initially</b> set as
	 *      the status bar icon and is expected to provide a background image
	 *      (defaults to ''). Example usage:
	 * 
	 * <pre><code>
	 * // Example CSS rule:
    ...
    cls : 'x-statusbar',
    /**
	 * @cfg {String} busyIconCls The default <code>{@link #iconCls}</code>
	 *      applied when calling <code>{@link #showBusy}</code> (defaults to
	 *      <tt>'x-status-busy'</tt>). It can be overridden at any time by
	 *      passing the <code>iconCls</code> argument into
	 *      <code>{@link #showBusy}</code>.
	 */
    busyIconCls : 'x-status-busy',
    /**
	 * @cfg {String} busyText The default <code>{@link #text}</code> applied
	 *      when calling <code>{@link #showBusy}</code> (defaults to
	 *      <tt>'Loading...'</tt>). It can be overridden at any time by
	 *      passing the <code>text</code> argument into
	 *      <code>{@link #showBusy}</code>.
	 */
    busyText : 'Loading...',
    /**
	 * @cfg {Number} autoClear The number of milliseconds to wait after setting
	 *      the status via <code>{@link #setStatus}</code> before
	 *      automatically clearing the status text and icon (defaults to
	 *      <tt>5000</tt>). Note that this only applies when passing the
	 *      <tt>clear</tt> argument to <code>{@link #setStatus}</code> since
	 *      that is the only way to defer clearing the status. This can be
	 *      overridden by specifying a different <tt>wait</tt> value in
	 *      <code>{@link #setStatus}</code>. Calls to
	 *      <code>{@link #clearStatus}</code> always clear the status bar
	 *      immediately and ignore this value.
	 */
    autoClear : 5000,

    /**
	 * @cfg {String} emptyText The text string to use if no text has been set.
	 *      Defaults to <tt>'&nbsp;'</tt>). If there are no other items in
	 *      the toolbar using an empty string (<tt>''</tt>) for this value
	 *      would end up in the toolbar height collapsing since the empty string
	 *      will not maintain the toolbar height. Use <tt>''</tt> if the
	 *      toolbar should collapse in height vertically when no text is
...
Ext.ux.tree.XmlTreeLoader = Ext.extend(Ext.tree.TreeLoader, {
    ...
    /*
	 * Template method intended to be overridden by subclasses that need to
	 * provide custom attribute processing prior to the creation of each
	 * TreeNode. This method will be passed a config object containing existing
	 * TreeNode attribute name/value pairs which can be modified as needed
	 * directly (no need to return the object).
	 */
...
Ext.ux.ValidationStatus = Ext.extend(Ext.Component, {
    ...
    /**
	 * @cfg {String} submitText The {@link #text} value to be applied when the
	 *      form is being submitted. Defaults to <tt>'Saving...'</tt>.
	 */
    submitText : 'Saving...',
    ...
    showErrors : function(){
        ...
        this.form.getEl().on('click', this.hideErrors, this, {single:true}); // hide
																				// if
																				// the
																				// user
																				// clicks
