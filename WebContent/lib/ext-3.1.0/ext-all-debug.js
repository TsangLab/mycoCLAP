...
Ext.apply(Ext.EventObjectImpl.prototype, {
			...
			isNavKeyPress : function() {
				...
				return (k >= 33 && k <= 40) || k == me.RETURN || k == me.TAB
						|| k == me.ESC;
			},
...
Ext.Element.addMethods({
	...
	getAlignToXY : function(el, p, o) {
		...
		w, h, r, dw = Ext.lib.Dom.getViewWidth() - 10, dh = Ext.lib.Dom
				.getViewHeight()
				- 10, p1y, p1x, p2y, p2x, swapY, swapX, doc = document, docElement = doc.documentElement, docBody = doc.body, scrollX = (docElement.scrollLeft
...
Ext.Element.addMethods({
	...
	scrollTo : function(side, value, animate) {
		...
		if (!animate || !me.anim) {
			prop = 'scroll' + (top ? 'Top' : 'Left'), dom[prop] = value;
		} else {
			prop = 'scroll' + (top ? 'Left' : 'Top'), me.anim({
						scroll : {
	...
	getWeekOfYear : function() {
		...
		return function() {
			var DC3 = Date.UTC(this.getFullYear(), this.getMonth(), this
							.getDate()
							+ 3)
					/ ms1d, AWN = Math.floor(DC3 / 7), Wyr = new Date(AWN
					* ms7d).getUTCFullYear();
...
Ext.layout.ColumnLayout = Ext.extend(Ext.layout.ContainerLayout, {
			...
			onLayout : function(ct, target) {
				...
				for (i = 0; i < len; i++) {
					c = cs[i];
					if (!c.columnWidth) {
						pw -= (c.getSize().width + c.getPositionEl()
								.getMargins('lr'));
					}
				}
...
Ext.layout.AbsoluteLayout = Ext.extend(Ext.layout.AnchorLayout, {
			...
			adjustWidthAnchor : function(value, comp) {
				return value ? value - comp.getPosition(true)[0]
						+ this.paddingLeft : value;
			},

			adjustHeightAnchor : function(value, comp) {
				return value ? value - comp.getPosition(true)[1]
...
(function() {
	...
	Ext.dd.DragDrop.prototype = {
		...
		constrainTo : function(constrainTo, pad, inContent) {
			...
			var b = Ext.get(this.getEl()).getBox(), ce = Ext.get(constrainTo), s = ce
					.getScroll(), c, cd = ce.dom;
			if (cd == document.body) {
				c = {
					x : s.left,
					y : s.top,
...
Ext.data.ArrayReader = Ext.extend(Ext.data.JsonReader, {
	...
	readRecords : function(o) {
		this.arrayData = o;
		var s = this.meta, sid = s ? Ext.num(s.idIndex, s.id) : null, recordType = this.recordType, fields = recordType.prototype.fields, records = [], v;

		var root = this.getRoot(o);

		for (var i = 0, len = root.length; i < len; i++) {
			var n = root[i], values = {}, id = ((sid || sid === 0)
					&& n[sid] !== undefined && n[sid] !== "" ? n[sid] : null);
...
Ext.data.ArrayStore = Ext.extend(Ext.data.Store, {
			...
			loadData : function(data, append) {
				...
				Ext.data.ArrayStore.superclass.loadData
						.call(this, data, append);
			}
		});
...
Ext.extend(Ext.data.XmlReader, Ext.data.DataReader, {
	...
	readResponse : function(action, response) {
		...
		var res = new Ext.data.Response({
					...
					data : this.extractData(q.select(this.meta.record, doc)
									|| q.select(this.meta.root, doc), false),
					raw : doc
				});
...
Ext.data.GroupingStore = Ext.extend(Ext.data.Store, {
	...
	groupBy : function(field, forceRegroup, direction) {
		direction = direction ? (String(direction).toUpperCase() == 'DESC'
				? 'DESC'
				: 'ASC') : this.groupDir;
		if (this.groupField == field && this.groupDir == direction
				&& !forceRegroup) {
			return;
		}
	...
	applySort : function() {
		...
		if (!this.groupOnSort && !this.remoteGroup) {
			...
			if (gs
					&& (gs != this.sortInfo.field || this.groupDir != this.sortInfo.direction)) {
...
Ext.extend(Ext.data.DirectProxy, Ext.data.DataProxy, {
	...
	doRequest : function(action, rs, params, reader, callback, scope, options) {
		var args = [], directFn = this.api[action] || this.directFn;
...
Ext.data.DirectStore = function(c) {
	...
	Ext.data.DirectStore.superclass.constructor.call(this, Ext.apply(c, {
		proxy : (typeof(c.proxy) == 'undefined')
				? new Ext.data.DirectProxy(Ext.copyTo({}, c,
						'paramOrder,paramsAsHash,directFn,api'))
				: c.proxy,
		reader : (typeof(c.reader) == 'undefined' && typeof(c.fields) == 'object')
				? new Ext.data.JsonReader(Ext.copyTo({}, c,
								'totalProperty,root,idProperty'), c.fields)
				: c.reader
	}));
};
...
Ext.Direct = Ext.extend(Ext.util.Observable, {
			...
			createEvent : function(response, extraProps) {
				return new Ext.Direct.eventTypes[response.type](Ext.apply(
						response, extraProps));
			}
		});
...
Ext.direct.PollingProvider = Ext.extend(Ext.direct.JsonProvider, {
			...
			constructor : function(config) {
				Ext.direct.PollingProvider.superclass.constructor.call(this,
						config);
				this.addEvents(
...
Ext.direct.RemotingProvider = Ext.extend(Ext.direct.JsonProvider, {
	...
	initAPI : function() {
		var o = this.actions;
		for (var c in o) {
			var cls = this.namespace[c] || (this.namespace[c] = {}), ms = o[c];
	...
	onData : function(opt, success, xhr) {
		if (success) {
			var events = this.getEvents(xhr);
			for (var i = 0, len = events.length; i < len; i++) {
				var e = events[i], t = this.getTransaction(e);
				this.fireEvent('data', this, e);
				if (t) {
	...
	doSend : function(data) {
		...
		if (this.enableUrlEncode) {
			var params = {};
			params[Ext.isString(this.enableUrlEncode)
	...
	doCall : function(c, m, args) {
		...
		var t = new Ext.Direct.Transaction({
					...
					method : m.name,
					data : data,
					cb : scope && Ext.isFunction(hs)
							? hs.createDelegate(scope)
							: hs
				});
	...
	doForm : function(c, m, form, callback, scope) {
		var t = new Ext.Direct.Transaction({
					...
					cb : scope && Ext.isFunction(callback) ? callback
		...
		if (this.fireEvent('beforecall', this, t) !== false) {
			Ext.Direct.addTransaction(t);
			var isUpload = String(form.getAttribute("enctype")).toLowerCase() == 'multipart/form-data', params = {
				extTID : t.tid,
				extAction : c,
				extMethod : m.name,
				extType : 'rpc',
	...
	doCallback : function(t, e) {
		...
		if (t && t.cb) {
			var hs = t.cb, result = Ext.isDefined(e.result) ? e.result : e.data;
		...
		if (this.resizeChild) {
			...
			if (Ext.isIE) {
				setTimeout(function() {
							if (el.dom.offsetWidth) {
								var b = el.getSize(true);
								child.setSize(b.width + adj[0], b.height
												+ adj[1]);
							}
						}, 10);
			}
...
Ext.DataView = Ext.extend(Ext.BoxComponent, {
	...
	onBeforeLoad : function() {
		if (this.loadingText) {
			this.clearSelections(false, true);
			this.getTemplateTarget().update('<div class="loading-indicator">'
...
Ext.list.ListView = Ext.extend(Ext.DataView, {
	...
	initComponent : function() {
		...
		for (var i = 0; i < len; i++) {
			var c = cs[i];
			if (!c.isColumn) {
				c.xtype = c.xtype ? (/^lv/.test(c.xtype) ? c.xtype : 'lv'
						+ c.xtype) : 'lvcolumn';
				c = Ext.create(c);
			}
			if (c.width) {
	...
	collectData : function() {
		var rs = Ext.list.ListView.superclass.collectData
	...
	onResize : function(w, h) {
		...
		if (Ext.isNumber(w)) {
			...
			if (this.reserveScrollOffset
					|| ((bdp.offsetWidth - bdp.clientWidth) > 10)) {
...
Ext.list.NumberColumn = Ext.extend(Ext.list.Column, {
			...
			constructor : function(c) {
				c.tpl = c.tpl
						|| new Ext.XTemplate('{' + c.dataIndex + ':number("'
...
Ext.list.DateColumn = Ext.extend(Ext.list.Column, {
			...
			constructor : function(c) {
				c.tpl = c.tpl
						|| new Ext.XTemplate('{' + c.dataIndex + ':date("'
								+ (c.format || this.format) + '")}');
...
Ext.list.ColumnResizer = Ext.extend(Ext.util.Observable, {
	...
	handleHdMove : function(e, t) {
		var hw = 5, x = e.getPageX(), hd = e.getTarget('em', 3, true);
		if (hd) {
			...
			if (x - r.left <= hw && pn != pn.parentNode.firstChild) {
				...
				ss.cursor = Ext.isWebKit ? 'e-resize' : 'col-resize';
					...
					&& pn != pn.parentNode.lastChild.previousSibling) {
				this.activeHd = hd;
	...
	onStart : function(e) {
		...
		var x = this.tracker.getXY()[0], w = this.view.innerHd.getWidth();
	...
	onEnd : function(e) {
		...
		var index = this.hdIndex, vw = this.view, cs = vw.columns, len = cs.length, w = this.view.innerHd
				.getWidth(), minPct = this.minPct * 100, pct = Math
				.ceil((nw * vw.maxWidth) / w), diff = (cs[index].width * 100)
				- pct, each = Math.floor(diff / (len - 1 - index)), mod = diff
				- (each * (len - 1 - index));

		for (var i = index + 1; i < len; i++) {
			var cw = (cs[i].width * 100) + each, ncw = Math.max(minPct, cw);
			if (cw != ncw) {
				mod += cw - ncw;
			}
			cs[i].width = ncw / 100;
...
Ext.TabPanel = Ext.extend(Ext.Panel, {
	...
	afterRender : function() {
		...
		if (this.activeTab !== undefined) {
			var item = Ext.isObject(this.activeTab)
					? this.activeTab
	...
	initTab : function(item, index) {
		var before = this.strip.dom.childNodes[index], p = this
				.getTemplateArgs(item), el = before ? this.itemTpl
				.insertBefore(before, p) : this.itemTpl.append(this.strip, p), cls = 'x-tab-strip-over', tabEl = Ext
				.get(el);
	...
	autoSizeTabs : function() {
		var count = this.items.length, ce = this.tabPosition != 'bottom'
				? 'header'
				: 'footer', ow = this[ce].dom.offsetWidth, aw = this[ce].dom.clientWidth;
		...
		for (var i = 0, len = lis.length; i < len; i++) {
			var li = lis[i], inner = Ext.fly(li).child('.x-tab-strip-inner',
					true), tw = li.offsetWidth, iw = inner.offsetWidth;
	...
	autoScrollTabs : function() {
		...
		var count = this.items.length, ow = this.pos.dom.offsetWidth, tw = this.pos.dom.clientWidth, wrap = this.stripWrap, wd = wrap.dom, cw = wd.offsetWidth, pos = this
				.getScrollPos(), l = this.edge.getOffsetsTo(this.stripWrap)[0]
				+ pos;
	...
	scrollToTab : function(item, animate) {
		...
		var el = this.getTabEl(item), pos = this.getScrollPos(), area = this
				.getScrollArea(), left = Ext.fly(el)
				.getOffsetsTo(this.stripWrap)[0]
				+ pos, right = left + el.offsetWidth;
		if (left < pos) {
	...
	onWheel : function(e) {
		...
		var pos = this.getScrollPos(), newpos = pos + d, sw = this
				.getScrollWidth()
				- this.getScrollArea();
	...
	onScrollRight : function() {
		var sw = this.getScrollWidth() - this.getScrollArea(), pos = this
				.getScrollPos(), s = Math.min(sw, pos
	...
	onScrollLeft : function() {
		var pos = this.getScrollPos(), s = Math.max(0, pos
						- this.getScrollIncrement());
		if (s != pos) {
			this.scrollTo(s, this.animScroll);
		}
...
Ext.Button = Ext.extend(Ext.BoxComponent, {
	...
	initButtonEl : function(btn, btnEl) {
		...
		if (this.repeat) {
			var repeater = new Ext.util.ClickRepeater(btn, Ext
	...
	setIcon : function(icon) {
		this.icon = icon;
		if (this.el) {
			this.btnEl.setStyle('background-image', icon
							? 'url(' + icon + ')'
							: '');
			this.setButtonClass();
		}
...
Ext.layout.ToolbarLayout = Ext.extend(Ext.layout.ContainerLayout, {
	...
	onLayout : function(ct, target) {
		...
		for (var i = 0, len = items.length, c; i < len; i++, pos++) {
			...
			if (c.isFill) {
				...
				if (!c.xtbHidden
						&& !this.isValidParent(c, side.childNodes[pos])) {
	...
	getItemWidth : function(c) {
		return c.hidden
				? (c.xtbWidth || 0)
				: c.getPositionEl().dom.parentNode.offsetWidth;
	},
	...
	fitToSize : function(t) {
		...
		var w = t.dom.clientWidth, lw = this.lastWidth || 0, iw = t.dom.firstChild.offsetWidth, clipWidth = w
				- this.triggerWidth, hideIndex = -1;

		this.lastWidth = w;

		if (iw > w || (this.hiddens && w >= lw)) {
			var i, items = this.container.items.items, len = items.length, c, loopWidth = 0;
	...
	createMenuConfig : function(c, hideOnClick) {
		var cfg = Ext.apply({}, c.initialConfig), group = c.toggleGroup;
	...
	beforeMoreShow : function(m) {
		var h = this.container.items.items, len = h.length, c, prev, needsSep = function(
				group, item) {
			return group.isXType('buttongroup')
					&& !(item instanceof Ext.Toolbar.Separator);
		};
...
(function() {
	...
	Ext.extend(T, Ext.Container, {
				...
				lookupComponent : function(c) {
					if (Ext.isString(c)) {
						...
						if (c.isFormField || c.render) {
							...
							c = c.xtype ? this.createComponent(c) : this
									.constructButton(c);
						}
					}
					return c;
				...
				addElement : function(el) {
					return this.addItem(new T.Item({
								el : el
							}));
				},
				...
				insertButton : function(index, item) {
					...
					return Ext.Toolbar.superclass.insert
							.call(this, index, item);
				},

				trackMenu : function(item, remove) {
					if (this.trackMenus && item.menu) {
						var method = remove ? 'mun' : 'mon';
						this[method](item, 'menutriggerover',
								this.onButtonTriggerOver, this);
						this[method](item, 'menushow', this.onButtonMenuShow,
								this);
						this[method](item, 'menuhide', this.onButtonMenuHide,
								this);
					}
				},

				constructButton : function(item) {
					var b = item.events ? item : this.createComponent(item,
							item.split ? 'splitbutton' : this.defaultType);
					return b;
				},
	...
	T.TextItem = Ext.extend(T.Item, {
				...
				constructor : function(config) {
					T.TextItem.superclass.constructor.call(this, Ext
									.isString(config) ? {
								text : config
							} : config);
				},
...
Ext.ButtonGroup = Ext.extend(Ext.Panel, {
			...
			onAfterLayout : function() {
				var bodyWidth = this.body.getFrameWidth('lr')
...
Ext.Tip = Ext.extend(Ext.Panel, {
	...
	doAutoWidth : function(adjust) {
		...
		bw += this.getFrameWidth() + (this.closable ? 20 : 0)
				+ this.body.getPadding("lr") + adjust;
...
Ext.ToolTip = Ext.extend(Ext.Tip, {
	...
	getTargetXY : function() {
		...
		if (this.anchor) {
			this.targetCounter++;
			var offsets = this.getOffsets(), xy = (this.anchorToTarget && !this.trackMouse)
					? this.el.getAlignToXY(this.anchorTarget, this
									.getAnchorAlign())
					: this.targetXY, dw = Ext.lib.Dom.getViewWidth() - 5, dh = Ext.lib.Dom
					.getViewHeight()
					- 5, de = document.documentElement, bd = document.body, scrollX = (de.scrollLeft
					|| bd.scrollLeft || 0)
					+ 5, scrollY = (de.scrollTop || bd.scrollTop || 0) + 5, axy = [
					xy[0] + offsets[0], xy[1] + offsets[1]]
			sz = this.getSize();
			...
			return [this.targetXY[0] + mouseOffset[0],
		...
		if (this.anchorToTarget && !this.trackMouse) {
			...
			switch (ap) {
				case 't' :
					...
					break;
				case 'b' :
					offsets = [-19 - this.anchorOffset,
							-13 - this.el.dom.offsetHeight];
					break;
				case 'r' :
					offsets = [-15 - this.el.dom.offsetWidth,
...
Ext.QuickTip = Ext.extend(Ext.ToolTip, {
			...
			getTipCfg : function(e) {
				...
				if (this.interceptTitles && t.title && Ext.isString(t.title)) {
					...
					cfg = this.tagConfig;
					ttp = t.qtip
							|| Ext.fly(t).getAttribute(cfg.attribute,
									cfg.namespace);
				}
				return ttp;
			},
			...
			onTargetOver : function(e) {
				...
				if (this.activeTarget
						&& ((t == this.activeTarget.el) || Ext
								.fly(this.activeTarget.el).contains(t))) {
					this.clearTimer('hide');
					this.show();
					return;
				}
			...
			onTargetOut : function(e) {
				...
				if (this.activeTarget && e.within(this.activeTarget.el)
						&& !this.getTipCfg(e)) {
					return;
				}
...
Ext.tree.TreeEventModel.prototype = {
	...
	getNode : function(e) {
		...
		if (t = e.getTarget('.x-tree-node-el', 10)) {
			var id = Ext.fly(t, '_treeEvents').getAttribute('tree-node-id',
					'ext');
			if (id) {
			...
			selectNext : function(s) {
				...
				if (s.firstChild && s.isExpanded()
					...
					s.parentNode.bubble(function() {
								if (this.nextSibling) {
									newS = this.getOwnerTree().selModel.select(
...
Ext.extend(Ext.tree.TreeNode, Ext.data.Node, {
	...
	insertBefore : function(node, refNode) {
		...
		var newNode = Ext.tree.TreeNode.superclass.insertBefore.call(this,
				node, refNode);
...
Ext.tree.TreeNodeUI.prototype = {
	...
	renderElements : function(n, a, targetNode, bulkRender) {
		...
		var cb = Ext.isBoolean(a.checked), nel, href = a.href
				? a.href
				: Ext.isGecko ? "" : "#", buf = [
	...
	updateExpandIcon : function() {
		if (this.rendered) {
			var n = this.node, c1, c2, cls = n.isLast()
					? "x-tree-elbow-end"
					: "x-tree-elbow", hasChild = n.hasChildNodes();
...
Ext.tree.TreeSorter = function(tree, config) {
	...
	this.sortFn = function(n1, n2) {
		...
		var v1 = sortType ? sortType(n1.attributes[p]) : (cs
				? n1.attributes[p]
				: n1.attributes[p].toUpperCase());
		var v2 = sortType ? sortType(n2.attributes[p]) : (cs
				? n2.attributes[p]
		...
		onContainerDrop : function(dd, e, data) {
			if (this.allowContainerDrop && this.isValidDropPoint({
				...
				var dropNode = data.node
						|| (dd.getTreeNode ? dd.getTreeNode(data, targetNode,
								'append', e) : null);
				return this.processDrop(targetNode, data, 'append', dd, e,
						dropNode);
			}
			return false;
		},
...
Ext.FlashComponent.EXPRESS_INSTALL_URL = 'http:/'
...
Ext.chart.Chart = Ext.extend(Ext.FlashComponent, {
			...
			setTipRenderer : function(fn) {
				var chart = this;
				this.tipFnName = this.createFnProxy(function(item, index,
								series) {
						...
						for (i = 0; i < seriesCount; i++) {
							...
							for (var prop in currentSeries) {
								if (prop == "style"
									...
									clonedSeries.style = Ext
											.encode(currentSeries.style);
									styleChanged = true;
...
Ext.chart.Chart.CHART_URL = 'http:/'
		+ '/yui.yahooapis.com/2.7.0/build/charts/assets/charts.swf';
...
Ext.chart.CartesianChart = Ext.extend(Ext.chart.Chart, {
			onSwfReady : function(isReset) {
				Ext.chart.CartesianChart.superclass.onSwfReady.call(this,
						isReset);
	...
	doAutoSize : function() {
		...
		if (ct.floating) {
			if (w) {
				...
				ct.setWidth(Ext.isStrict && (Ext.isIE7 || Ext.isIE8)
						? 'auto'
						: ct.minWidth);
...
Ext.menu.Menu = Ext.extend(Ext.Container, {
	...
	onScroll : function(e, t) {
		...
		if (top
				? ul.scrollTop <= 0
				: ul.scrollTop + this.activeMax >= ul.scrollHeight) {
	...
	constrainScroll : function(y) {
		...
		if (this.floating) {
			max = this.maxHeight ? this.maxHeight : Ext
			...
			return {
				...
				doRelay : function(e, h) {
					...
					if (this.menu.activeItem
							&& this.menu.activeItem.isFormField && k != e.TAB) {
						return false;
					}
					if (!this.menu.activeItem && e.isNavKeyPress()
							&& k != e.SPACE && k != e.RETURN) {
...
Ext.form.DisplayField = Ext.extend(Ext.form.Field, {
			...
			getRawValue : function() {
				var v = this.rendered ? this.el.dom.innerHTML : Ext.value(
						this.value, '');
				if (v === this.emptyText) {
					v = '';
				}
...
Ext.grid.GridPanel = Ext.extend(Ext.Panel, {
	...
	applyState : function(state) {
		var cm = this.colModel, cs = state.columns;
		if (cs) {
			for (var i = 0, len = cs.length; i < len; i++) {
				var s = cs[i], c = cm.getColumnById(s.id);
				if (c) {
					c.hidden = s.hidden;
					c.width = s.width;
	...
	processEvent : function(name, e) {
		...
		var t = e.getTarget(), v = this.view, header = v.findHeaderIndex(t);

		if (header !== false) {
			this.fireEvent('header' + name, this, header, e);
		} else {
			var row = v.findRowIndex(t), cell, body;
			if (row !== false) {
	...
	walkCells : function(row, col, step, fn, scope) {
		var cm = this.colModel, clen = cm.getColumnCount(), ds = this.store, rlen = ds
				.getCount(), first = true;
		if (step < 0) {
			if (col < 0) {
				row--;
...
Ext.grid.GridView = Ext.extend(Ext.util.Observable, {
	...
	findRowBody : function(el) {
		...
		return this.fly(el).findParent(this.rowBodySelector,
		...
		for (var j = 0, len = rs.length; j < len; j++) {
			...
			for (var i = 0; i < colCount; i++) {
				...
				p.css = i === 0 ? 'x-grid3-cell-first ' : (i == last
						...
						: '');
				p.attr = p.cellAttr = '';
				p.value = c.renderer.call(c.scope, r.data[c.name], p, r,
						rowIndex, i, ds);
				p.style = c.style;
	...
	renderHeaders : function() {
		var cm = this.cm, ts = this.templates, ct = ts.hcell, cb = [], p = {}, len = cm
	...
	ensureVisible : function(row, col, hscroll) {
		...
		var cbot = ctop + rowEl.offsetHeight, ch = c.clientHeight, sbot = stop
				+ ch;
	...
	insertRows : function(dm, firstRow, lastRow, isUpdate) {
		...
		if (!isUpdate && firstRow === 0 && lastRow >= last) {
			...
			if (!isUpdate) {
				...
				Ext.fly(this.getRow(firstRow)).addClass(firstRow === 0
						? this.firstRowCls
						: this.lastRowCls);
			}
		}
	...
	getColumnWidth : function(col) {
		...
		if (Ext.isNumber(w)) {
			return (Ext.isBorderBox || (Ext.isWebKit && !Ext.isSafari2)
					? w
					: (w - this.borderWidth > 0 ? w - this.borderWidth : 0))
					+ 'px';
		}
		return w;
	},
...
Ext.grid.HeaderDropZone = Ext.extend(Ext.dd.DropZone, {
			...
			constructor : function(grid, hd, hd2) {
				...
				Ext.grid.HeaderDropZone.superclass.constructor.call(this, grid
								.getGridEl().dom);
			},
	...
	onEditorKey : function(field, e) {
		...
		if (k == e.TAB) {
			...
			if (this.moveEditorOnEnter !== false) {
				if (shift) {
					newCell = g.walkCells(last.row - 1, last.col, -1,
...
Ext.grid.TemplateColumn = Ext.extend(Ext.grid.Column, {
			...
			constructor : function(cfg) {
				...
				var tpl = (!Ext.isPrimitive(this.tpl) && this.tpl.compile)
						? this.tpl
...
Ext.grid.CellSelectionModel = Ext.extend(Ext.grid.AbstractSelectionModel, {
	...
	handleKeyDown : function(e) {
		...
		var k = e.getKey(), g = this.grid, s = this.selection, sm = this, walk = function(
				row, col, step) {
			return g.walkCells(row, col, step, g.isEditor && g.editing
							? sm.acceptsNav
							: sm.isSelectable, sm);
		}, cell, newCell, r, c, ae;
...
Ext.grid.EditorGridPanel = Ext.extend(Ext.grid.GridPanel, {
			...
			initEvents : function() {
				...
				if (this.clicksToEdit == 1) {
					...
					if (this.clicksToEdit == 'auto' && view.mainBody) {
						view.mainBody.on('mousedown', this.onAutoEditClick,
								this);
					}
			...
			onResize : function() {
				Ext.grid.EditorGridPanel.superclass.onResize.apply(this,
						arguments);
			...
			onAutoEditClick : function(e, t) {
				...
				var row = this.view.findRowIndex(t), col = this.view
						.findCellIndex(t);
...
Ext.grid.PropertyGrid = Ext.extend(Ext.grid.EditorGridPanel, {
			...
			initComponent : function() {
				...
				this.mon(this.selModel, 'beforecellselect', function(sm,
								rowIndex, colIndex) {
							if (colIndex === 0) {
...
Ext.grid.GroupingView = Ext.extend(Ext.grid.GridView, {
	...
	toggleGroup : function(group, expanded) {
		...
		expanded = Ext.isDefined(expanded) ? expanded : gel
	...
	renderRows : function() {
		...
		if (this.hideGroupedColumn) {
			var colIndex = this.cm.findColumnIndex(groupField), hasLastGroupField = Ext
	...
	doRender : function(cs, rs, ds, startRow, colCount, stripe) {
		...
		this.enableGrouping = (this.enableGrouping === false)
				? false
				: !!groupField;
		...
		var gstyle = 'width:' + this.getTotalWidth() + ';', cfg = this.cm.config[colIndex], groupRenderer = cfg.groupRenderer
				|| cfg.renderer, prefix = this.showGroupName
				? (cfg.groupName || cfg.header) + ': '
				: '', groups = [], curGroup, i, len, gid;
	...
	constructId : function(value, field, idx) {
		var cfg = this.cm.config[idx], groupRenderer = cfg.groupRenderer
				|| cfg.renderer, val = (this.groupMode == 'value')
				? value
				: this.getGroup(value, {
							data : {}
						}, groupRenderer, 0, idx, this.ds);
